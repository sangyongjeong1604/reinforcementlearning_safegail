...
class PPOTrain:
    ...
    def train(self, sess, obs, actions, gaes, rewards, v_preds_next):
        sess.run(self.train_op, feed_dict={self.Policy.obs: obs,
                                                               self.Old_Policy.obs: obs,
                                                               self.actions: actions,
                                                               self.rewards: rewards,
                                                               self.v_preds_next: v_preds_next,
                                                               self.gaes: np.squeeze(gaes)})

    def get_summary(self, sess, obs, actions, gaes, rewards, v_preds_next):
        return sess.run(self.merged, feed_dict={self.Policy.obs: obs,
                                                                    self.Old_Policy.obs: obs,
                                                                    self.actions: actions,
                                                                    self.rewards: rewards,
                                                                    self.v_preds_next: v_preds_next,
